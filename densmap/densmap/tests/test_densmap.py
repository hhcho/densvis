...
from densmap.utils import deheap_sort
from densmap.nndescent import (
    make_initialisations,
    make_initialized_nnd_search,
    initialise_search,
)
import densmap.validation as valid
import densmap.sparse as spdist
import densmap.distances as dist
...
"""
Tests for DENSMAP to ensure things are working as expected.
"""
import warnings
...
def test_densmap_sparse_trustworthiness():
    embedding = DENSMAP(n_neighbors=10).fit_transform(sparse_nn_data[:100])
...
def test_densmap_trustworthiness_on_iris():
    data = iris.data
    embedding = DENSMAP(n_neighbors=10, min_dist=0.01, random_state=42).fit_transform(data)
...
def test_densmap_trustworthiness_on_iris_random_init():
    data = iris.data
    embedding = DENSMAP(
...
def test_supervised_densmap_trustworthiness_on_iris():
    data = iris.data
    embedding = DENSMAP(n_neighbors=10, min_dist=0.01, random_state=42).fit_transform(
        data, iris.target
    )
    trust = trustworthiness(iris.data, embedding, 10)
    assert_greater_equal(
...
def test_semisupervised_densmap_trustworthiness_on_iris():
    data = iris.data
...
def test_initialized_densmap_trustworthiness_on_iris():
    data = iris.data
    embedding = DENSMAP(
...
def test_densmap_transform_on_iris():
    data = iris.data[iris_selection]
    fitter = DENSMAP(n_neighbors=10, min_dist=0.01, random_state=42).fit(data)
...
def test_densmap_transform_on_iris_modified_dtype():
    data = iris.data[iris_selection]
    fitter = DENSMAP(n_neighbors=10, min_dist=0.01, random_state=42).fit(data)
...
# @SkipTest
# def test_densmap_regression_supervision(): # pragma: no cover
#     boston = datasets.load_boston()
#     data = boston.data
#     embedding = DENSMAP(n_neighbors=10,
#                      min_dist=0.01,
...
def test_multi_component_layout():
    ...
    embedding = DENSMAP(n_neighbors=4).fit_transform(data)
...
def test_negative_min_dist():
    u = DENSMAP(min_dist=-1)
    assert_raises(ValueError, u.fit, nn_data)


def test_negative_ncomponents():
    u = DENSMAP(n_components=-1)
    assert_raises(ValueError, u.fit, nn_data)


def test_non_integer_ncomponents():
    u = DENSMAP(n_components=1.5)
    assert_raises(ValueError, u.fit, nn_data)


def test_too_small_nneighbors():
    u = DENSMAP(n_neighbors=0.5)
    assert_raises(ValueError, u.fit, nn_data)


def test_negative_nneighbors():
    u = DENSMAP(n_neighbors=-1)
    assert_raises(ValueError, u.fit, nn_data)
...
def test_negative_learning_rate():
    u = DENSMAP(learning_rate=-1.5)
    assert_raises(ValueError, u.fit, nn_data)


def test_negative_repulsion():
    u = DENSMAP(repulsion_strength=-0.5)
    assert_raises(ValueError, u.fit, nn_data)


def test_negative_sample_rate():
    u = DENSMAP(negative_sample_rate=-1)
    assert_raises(ValueError, u.fit, nn_data)


def test_bad_init():
    u = DENSMAP(init="foobar")
    assert_raises(ValueError, u.fit, nn_data)
...
def test_bad_matrix_init():
    u = DENSMAP(init=np.array([[0, 0, 0], [0, 0, 0]]))
    assert_raises(ValueError, u.fit, nn_data)


def test_negative_nepochs():
    u = DENSMAP(n_epochs=-2)
    assert_raises(ValueError, u.fit, nn_data)


def test_negative_target_nneighbors():
    u = DENSMAP(target_n_neighbors=1)
    assert_raises(ValueError, u.fit, nn_data)


def test_densmap_bad_nn():
    assert_raises(ValueError, nearest_neighbors, nn_data, 10, 42, {}, False, np.random)


def test_densmap_bad_nn_sparse():
    assert_raises(
        ValueError,
        nearest_neighbors,
        sparse_nn_data,
...
def test_too_many_neighbors_warns():
    u = DENSMAP(a=1.2, b=1.75, n_neighbors=2000, n_epochs=11, init="random")
    u.fit(nn_data[:100,])
    assert_equal(u._a, 1.2)
    assert_equal(u._b, 1.75)


def test_densmap_fit_params():
...
def test_densmap_transform_embedding_stability():
    ...
    trained DENSMAP transformer causes the fitting embedding matrix to change
    ...
    densmap = DENSMAP()
    u1 = densmap.fit_transform(a[:, :5])
    u1_orig = u1.copy()
    assert_array_equal(u1_orig, densmap.embedding_)

    u2 = densmap.transform(b)
    assert_array_equal(u1_orig, densmap.embedding_)
